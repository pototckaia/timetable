...
/*SELECT r1.id, r2.id
FROM SCHED_ITEMS r1
inner join SCHED_ITEMS r2
on r1.id != r2.id and 
    ((r1.AUDIENCE_ID is null and r2.AUDIENCE_ID is null) or r1.AUDIENCE_ID = r2.AUDIENCE_ID) and 
    ((r1.LESSON_ID is null and r2.LESSON_ID is null) or r1.LESSON_ID = r2.LESSON_ID) and 
    ((r1.SUBJECT_ID is null and r2.subject_id is null) or r1.subject_id = r2.subject_id) and 
    ((r1.GROUP_ID is null and r2.GROUP_ID is null) or r1.GROUP_ID = r2.GROUP_ID) and 
    ((r1.teacher_id is null and r2.teacher_id is null) or r1.teacher_id = r2.teacher_id) and 
    ((r1.type_id is null and r2.TYPE_ID is null) or r1.TYPE_ID = r2.TYPE_ID) and 
    ((r1.weekday_id is null and r2.weekday_id is null) or r1.weekday_id= r2.WEEKDAY_ID)

insert into CONFLICT_IN_SCHED (id_row_to, id_row_from, 2)
SELECT r1.id, r2.id
FROM SCHED_ITEMS r1
inner join SCHED_ITEMS r2
on r1.id != r2.id and 
    (r1.GROUP_ID is not null and r2.GROUP_ID is not null and r1.GROUP_ID = r2.GROUP_ID) and    
    (r1.LESSON_ID is not null and r2.LESSON_ID is not null and r1.LESSON_ID = r2.LESSON_ID) and 
    (r1.weekday_id is not null and r2.weekday_id is not null and r1.weekday_id = r2.WEEKDAY_ID)


insert into CONFLICT_IN_SCHED (id_row_to, id_row_from, id_type)
SELECT r1.id, r2.id, 3
FROM SCHED_ITEMS r1
inner join SCHED_ITEMS r2
on r1.id != r2.id and 
    (r1.TEACHER_ID is not null and r2.TEACHER_ID is not null and r1.TEACHER_ID = r2.TEACHER_ID) and    
    (r1.LESSON_ID is not null and r2.LESSON_ID is not null and r1.LESSON_ID = r2.LESSON_ID) and 
    (r1.weekday_id is not null and r2.weekday_id is not null and r1.weekday_id = r2.WEEKDAY_ID)


insert into CONFLICT_IN_SCHED (id_row_to, id_row_from, id_type)
SELECT r1.id, r2.id, 4
FROM SCHED_ITEMS r1
inner join SCHED_ITEMS r2
on r1.id != r2.id and 
    (r1.AUDIENCE_ID is not null and r2.AUDIENCE_ID is not null and r1.AUDIENCE_ID = r2.AUDIENCE_ID) and    
    (r1.LESSON_ID is not null and r2.LESSON_ID is not null and r1.LESSON_ID = r2.LESSON_ID) and 
    (r1.weekday_id is not null and r2.weekday_id is not null and r1.weekday_id = r2.WEEKDAY_ID)
...
RECREATE TABLE CONFLICT_IN_SCHED
(
  ID_ROW_TO Integer NOT NULL,
  ID_ROW_FROM Integer NOT NULL,
  ID_TYPE Integer
);
...
ALTER TABLE CONFLICT_IN_SCHED ADD CONSTRAINT FK_CONFLICT_1
  FOREIGN KEY (ID_ROW_TO) REFERENCES SCHED_ITEMS (ID);
  FOREIGN KEY (ID_ROW_FROM) REFERENCES SCHED_ITEMS (ID);
  FOREIGN KEY (ID_TYPE) REFERENCES SCHED_ITEMS (ID;
...
RECREATE TRIGGER DELETE_FROM_CONFLICT
    ACTIVE AFTER DELETE POSITION 0 ON SCHED_ITEMS
AS
BEGIN
   DELETE FROM CONFLICT_IN_SCHED as conf
   WHERE conf.ID_ROW_TO = old.ID or conf.ID_ROW_FROM = old.ID;
END :
    
RECREATE TRIGGER INSERT_FROM_CONFLICT
    ACTIVE AFTER INSERT POSITION 0 ON SCHED_ITEMS
AS
BEGIN
  FOR SELECT * 
    FROM SCHED_ITEMS
  ...
  BEGIN
    ...
    IF (new.ID != cur_row.ID and 
        ...
        ((new.LESSON_ID is null and cur_row.LESSON_ID is null) or new.LESSON_ID = cur_row.LESSON_ID) and 
        ((new.SUBJECT_ID is null and cur_row.subject_id is null) or new.subject_id = cur_row.subject_id) and 
        ((new.GROUP_ID is null and cur_row.GROUP_ID is null) or new.GROUP_ID = cur_row.GROUP_ID) and 
        ((new.TEACHER_ID is null and cur_row.TEACHER_ID is null) or new.TEACHER_ID = cur_row.TEACHER_ID) and 
        ((new.TYPE_ID is null and cur_row.TYPE_ID is null) or new.TYPE_ID = cur_row.TYPE_ID) and 
        ((new.WEEKDAY_ID is null and cur_row.WEEKDAY_ID is null) or new.WEEKDAY_ID = cur_row.WEEKDAY_ID))
    THEN 
    BEGIN
      INSERT INTO CONFLICT_IN_SCHED (ID_ROW_TO, ID_ROW_FROM, ID_TYPE) values (new.ID, cur_row.ID, 1);
      INSERT INTO CONFLICT_IN_SCHED (ID_ROW_TO, ID_ROW_FROM, ID_TYPE) values (cur_row.ID, new.ID, 1);
    END
    
    IF (new.ID != cur_row.ID and 
        (new.GROUP_ID is not null and cur_row.GROUP_ID is not null and new.GROUP_ID = cur_row.GROUP_ID) and    
        (new.LESSON_ID is not null and cur_row.LESSON_ID is not null and new.LESSON_ID = cur_row.LESSON_ID) and 
        (new.WEEKDAY_ID is not null and cur_row.WEEKDAY_ID is not null and new.WEEKDAY_ID = cur_row.WEEKDAY_ID))
    THEN
    BEGIN
      INSERT INTO CONFLICT_IN_SCHED (ID_ROW_TO, ID_ROW_FROM, ID_TYPE) values (new.ID, cur_row.ID, 2);
      INSERT INTO CONFLICT_IN_SCHED (ID_ROW_TO, ID_ROW_FROM, ID_TYPE) values (cur_row.ID, new.ID, 2);
    END
    
    IF (new.ID != cur_row.ID and 
        (new.TEACHER_ID is not null and cur_row.TEACHER_ID is not null and new.TEACHER_ID = cur_row.TEACHER_ID) and    
        (new.LESSON_ID is not null and cur_row.LESSON_ID is not null and new.LESSON_ID = cur_row.LESSON_ID) and 
        (new.WEEKDAY_ID is not null and cur_row.WEEKDAY_ID is not null and new.WEEKDAY_ID = cur_row.WEEKDAY_ID))
    THEN
    BEGIN
      INSERT INTO CONFLICT_IN_SCHED (ID_ROW_TO, ID_ROW_FROM, ID_TYPE) values (new.ID, cur_row.ID, 3);
      INSERT INTO CONFLICT_IN_SCHED (ID_ROW_TO, ID_ROW_FROM, ID_TYPE) values (cur_row.ID, new.ID, 3);
    END
        
    IF (new.ID != cur_row.ID and 
        (new.AUDIENCE_ID is not null and cur_row.AUDIENCE_ID is not null and new.AUDIENCE_ID = cur_row.AUDIENCE_ID) and    
        (new.LESSON_ID is not null and cur_row.LESSON_ID is not null and new.LESSON_ID = cur_row.LESSON_ID) and 
        (new.WEEKDAY_ID is not null and cur_row.WEEKDAY_ID is not null and new.WEEKDAY_ID = cur_row.WEEKDAY_ID))
    THEN
    BEGIN
        INSERT INTO CONFLICT_IN_SCHED (ID_ROW_TO, ID_ROW_FROM, ID_TYPE) values (new.ID, cur_row.ID, 4);
        INSERT INTO CONFLICT_IN_SCHED (ID_ROW_TO, ID_ROW_FROM, ID_TYPE) values (cur_row.ID, new.ID, 4);
    END
...
RECREATE TRIGGER UPDATE_FROM_CONFLICT
    ACTIVE AFTER UPDATE POSITION 0 ON SCHED_ITEMS
AS
BEGIN
  DELETE FROM CONFLICT_IN_SCHED as conf
    WHERE conf.ID_ROW_TO = old.ID or conf.ID_ROW_FROM = old.ID;  
...
INSERT INTO TYPE_CONFLICT (NAME, ORDER_NUMBER) values ('Дубликаты', 1);
INSERT INTO TYPE_CONFLICT (NAME, ORDER_NUMBER) values ('У группы совпадают занятия', 2);
INSERT INTO TYPE_CONFLICT (NAME, ORDER_NUMBER) values ('У преподавателя совпадают занятия', 3);
INSERT INTO TYPE_CONFLICT (NAME, ORDER_NUMBER) values ('В одной аудитории несколько занятия',  4); 
